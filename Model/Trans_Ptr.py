...
class Trans_Ptr(model):
    def __init__(self, game_name, path, config, loss_name, attribute = "reserve", copy_model = None):
        ...
        self.max_par_size = config.max_par_size
        self.out_size = config.max_par_size + 1
        self.action_size = self.out_size * self.max_task_size
        ...
        for i in range(self.batch_size):
            for j in range(self.max_task_size):
                for k in range(self.out_size):
                        sparse_action[i][j][action[i][j]] = 1
    ...
    def train_model(self, data, action, reward, value, next_obs, epoch, direct = False):
        ...
        print "sparse_action"
        feed = {self.tf_par : par_fea, self.tf_task : task_fea, self.task_all : task_all, self.initial_state_c: self.lstm_state[0], self.initial_state_h: self.lstm_state[1], self.tf_action : sparse_action, self.tf_epr: reward, self.keep_prob : 0.5, self.is_training : True, self.tf_Q : value}
    ...
    def _optimize(self, multi_task = True):
        ...
        self.loss = self._loss_f("CE") + pre_loss * 0.01 + self._loss_f("MSE") * self.value_coffe
