...
class env_trajectory(env):
    ...
    def preprocess(self):
        ...
        self.simulator.update_state(self.task_samples, [], self.config.dropout_lamda)
    ...
    def step_raw(self, actions):
        ...
        self.speed_tune()
        self.task_sampling()
        self.simulator.update_state(self.task_samples, actions, self.config.dropout_lamda)
        #self.simulator.output_state(self.config.log_file_path, self.inner_step)
        done = False
    ...
    def step(self, actions_pid_list, is_test=False):
        ...
        is_finished = dup_simulator.update_state([], [], self.config.dropout_lamda)
        while_counter = 0
        ...
        no_fair_reward = dup_simulator.reward
        task_time_cost = []
        par_fare_amount = []
        for tid in dup_simulator.task_pending_time:
            #print "TASK %s,%s" % (tid, self.simulator.task_pending_time[tid])
            task_time_cost.append(dup_simulator.task_pending_time[tid])
        for pid in dup_simulator.participant_fare:
            #print "PAR %s,%s" % (pid, self.simulator.participant_fare[pid])
            par_fare_amount.append(dup_simulator.participant_fare[pid])
        if len(task_time_cost) == 0:
            mean_time_cost = 0.0
            std_time_cost = 0.0
        else:
            mean_time_cost = np.mean(task_time_cost)
            std_time_cost = np.std(task_time_cost)
        if len(par_fare_amount) == 0:
            mean_fare_amount = 0.0
            std_fare_amount = 0.0
        else:
        ...
        self.simulator.final_reward = dup_simulator.reward - std_time_cost - std_fare_amount
        print "fair_reward:%s\t%s" % (no_fair_reward, self.simulator.final_reward)
