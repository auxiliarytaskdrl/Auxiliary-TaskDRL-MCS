        ...
        for file in files:
            ...
            for line in file:
                try:
                    ...
                    # find max min
                    if -80 <= start_x <= -70 and -80 <= end_x <= -70 \
                            and 35 <= start_y <= 45 and 35 <= end_y <= 45:
                        if x_min > start_x: x_min = start_x
                        if x_min > end_x: x_min = end_x
                        if x_max < start_x: x_max = start_x
                        if x_max < end_x: x_max = end_x
                        if y_min > start_y: y_min = start_y
                        if y_min > end_y: y_min = end_y
                        if y_max < start_y: y_max = start_y
                        if y_max < end_y: y_max = end_y
            ...
            file.close()
        print "TPOS bx,sx,by,sy, %s,%s,%s,%s" % (x_max,x_min,y_max,y_min)
        # -73.4165267944,-74.4095687866,41.0447845459,40.4790916443
