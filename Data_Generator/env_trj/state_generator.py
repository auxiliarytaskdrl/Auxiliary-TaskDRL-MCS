...
class Trajectory():
    def __init__(self):
        ...
        self.pos_range = [-73, -75, 42, 40]#[-73.4165267944, -74.4095687866, 41.0447845459, 40.4790916443] # x_max, x_min, y_max, y_min
        self.speed_seed = []
    ...
    def set_speed_seed(self, seed_size=100): 
        range_size = 0.8
        rand_t = [random.random() for i in range(seed_size)]
        for seed in rand_t:
            speed = (1 + (seed - 0.5) * 2 * range_size)# 1 + [-0.8 + 0.8]
            self.speed_seed.append(speed)
        #self.speed_seed = [(1 + (seed - 0.5) * 2 * range_size) for seed in rand_t] 
        self.speed_seed = [0.2, 0.8, 1.2, 1.8]
        return self.speed_seed
    ...
    def speed_pos_tune(self, positon, speed_raw):
        pos_x, pos_y = positon
        grid_size = 2
        x_max, x_min, y_max, y_min = self.pos_range
        x_index = int(grid_size * (pos_x - x_min) / (x_max - x_min))
        y_index = int(grid_size * (pos_y - y_min) / (y_max - y_min))
        if y_index >= grid_size:
            #print "y out of index:posx,y %s,%s" % (pos_x, pos_y)
            y_index = grid_size - 1
        if x_index >= grid_size:
            #print "x out of index:posx,y %s,%s" % (pos_x, pos_y)
            x_index = grid_size - 1
        tune_index = (y_index * grid_size) + x_index
        tune = self.speed_seed[tune_index]
        #print "%s,%s, speed seed index:%s" % (x_index, y_index, tune_index)
        new_speed = speed_raw * tune
        #new_speed = speed_raw
        #print "POS %s,%s  index:%s,%s  speed:%s,tune:%s  new speed:%s" % (pos_x, pos_y, x_index, y_index, speed_raw, tune, new_speed)
        return new_speed
    ...
    def move_simulate(self, start_pos, target_pos, speed_raw, unit_time=300):
        x1, y1 = start_pos
        x2, y2 = target_pos
        x3, y3 = start_pos
        speed = self.speed_pos_tune(start_pos, speed_raw)
        #print "SPEED: %s" % (speed)
        distance = self.get_distance(x1, y1, x2, y2)
        move_dis = speed * unit_time
        #print "DIS:%s, MOVE:%s" % (distance, move_dis)
...
class StateSimulator():
    ...
    def update_state(self, new_tasks, actions, droup_out_lamda=0.5, unit_time=300):
        """
        2.update positon
        3.execute action
        1.update new tasks
        ...
        #print "%s, %s" % (len(self.running_schedules), len(self.pending_actions))
        if len(self.running_schedules) == 0 \
                and len(self.pending_actions) == 0:
            is_finished = True
        self.update_position(unit_time)
        self.droup_out_task(droup_out_lamda)
        self.update_new_tasks(new_tasks)
        return is_finished

    def droup_out_task(self, lamda=0.5):
        pending_list = []
        for tid in self.pending_schedules:
            pending_list.append(tid)
        for tid in pending_list:
            if tid in self.task_pending_time:
                pend_round = float(self.task_pending_time[tid]) / self.TIME_SPEND_UNIT
                droup_out_prob = 1 - math.exp(0 - lamda * pend_round)
                #print "tid %s, pend_round %s, droup_out_prob %s" % (tid, pend_round, droup_out_prob)
                rand_p = random.random()
                if rand_p <= droup_out_prob: # droup out
                    self.pending_schedules.pop(tid)
                    self.task_pending_time.pop(tid)
                    #print "droup_out: %s, %s, %s" % (tid, rand_p, droup_out_prob)
                    continue
    ...
    def execution(self, action):
        ...
        if t_id not in self.pending_schedules:
            return True
        if p_id in self.participants \
            ...
            if t_id in self.task_pending_time:
                self.task_pending_time.pop(t_id)
    ...
    def reward_compute(self):
        ...
        for tid in self.pending_schedules:
            self.reward -= self.TIME_SPEND_UNIT * 5
            #print "-0.01",
            self.pending_time += self.TIME_SPEND_UNIT
            if tid in self.task_pending_time:
                self.task_pending_time[tid] += self.TIME_SPEND_UNIT
            else:
                self.task_pending_time[tid] = self.TIME_SPEND_UNIT
        #print "==%s" % self.reward
        return self.reward
    ...
    def update_new_tasks(self, new_tasks):
        ...
        for task_data in new_tasks:
            ...
            reward_state = 0                                        #reward计算状态[9],pending round number
